...
function ToastItem({ toast, onRemove }: ToastItemProps) {
  ...
  const getToastStyles = () => {
    const base = "border-l-4 rounded-lg shadow-lg";
    switch (toast.type) {
      case 'success':
  ...
  return (
    <div
      className={`${getToastStyles()} p-4 max-w-sm mx-auto transition-all duration-150 transform ${
    ...
    >
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className={`${getTextColor()} font-medium text-sm`}>
            {toast.title}
          </h4>
          {toast.message && (
            <p className={`${getTextColor()} text-sm opacity-75 mt-1`}>
              {toast.message}
            </p>
          )}
        </div>
        <button
          onClick={handleClose}
          className={`${getTextColor()} opacity-50 hover:opacity-75 flex-shrink-0`}
        >
